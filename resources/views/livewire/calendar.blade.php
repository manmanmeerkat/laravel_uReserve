<div>
    <div class="text-center text-sm">
       日付を選択してください。本日から最大30日先まで選択可能です。 
        ...
        <x-input id="calendar" class="block mt-1 mb-2 mx-auto" 
                ...
                @for($j = 0; $j < 21; $j++)
                    @if($events->isNotEmpty())
                      ...
                      $currentWeek[$i]['checkDay'] . " " . \Constant::EVENT_TIME[$j])))
                        ...
                        @endphp 
                        <div class="py-1 px-2 h-8 border boder-gray-200 text-xs bg-blue-100">
                            {{ $eventName }}
                        </div>
                        @if( $eventPeriod > 0)
                          @for($k = 0; $k < $eventPeriod ; $k++)
                          <div class="py-1 px-2 h-8 border boder-gray-200  bg-blue-100"></div>
                          @endfor
                          @php $j += $eventPeriod @endphp
                        @endif
                      @else
                        <div class="py-1 px-2 h-8 border boder-gray-200"></div>
